...
def diagnose():
    ...
    try:
        ...
        **reason 작성 규칙:**
        ...
        - 지침서의 어떤 부분이 부족한지 구체적으로 명시
        - 다른 항목의 내용을 절대 포함하지 말 것
        
        **countermeasure 작성 규칙:**
        - rating이 'N' 또는 'P'인 경우: 구체적이고 실행 가능한 개선방안 제시
        - rating이 'Y'인 경우: "현재 수준 유지" 또는 빈 문자열
        - 반드시 해당 항목에 특화된 대응방안 작성
        - 예시:
          - N 등급: "데이터베이스 접근 권한 관리 정책을 수립하고, 사용자별 접근 권한을 최소 권한 원칙에 따라 설정해야 합니다."
          - P 등급: "암호화 정책 문서를 보완하여 암호 알고리즘 선정 기준과 키 관리 절차를 명확히 해야 합니다."
          - Y 등급: "현재 수준 유지"
        ...
        **101개 항목 목록 (반드시 이 순서대로):**
        ...
        예시:
        {{"id": "2.6.4", "name": "데이터베이스 접근", "rating": "N", "reason": "지침서에 데이터베이스 접근 통제에 대한 명확한 정책이 명시되어 있지 않음", "countermeasure": "데이터베이스 접근 권한 관리 정책을 수립하고, 사용자별 최소 권한 원칙에 따라 접근 권한을 설정해야 합니다. 정기적인 권한 검토 절차도 포함하세요."}}
        {{"id": "2.7.1", "name": "암호정책 적용", "rating": "P", "reason": "암호화 정책이 일부 수립되어 있으나 암호 알고리즘 선정 기준이 불명확함", "countermeasure": "암호화 정책 문서를 보완하여 국가정보원 검증 암호 알고리즘 사용 기준과 키 관리 절차를 명확히 문서화해야 합니다."}}
        
        절대로 reason과 countermeasure 내용을 다른 항목과 섞지 마세요.
        ...
        **배열에는 정확히 101개의 객체가 포함되어야 합니다.**
        ...
        [JSON 응답 형식]
        [
          {{"id": "1.1.1", "name": "경영진의 참여", "rating": "Y/P/N", "reason": "평가 근거", "countermeasure": "대응방안"}},
          {{"id": "1.1.2", "name": "최고책임자의 지정", "rating": "Y/P/N", "reason": "평가 근거", "countermeasure": "대응방안"}},
          ...
          {{"id": "3.5.3", "name": "정보주체에 대한 통지", "rating": "Y/P/N", "reason": "평가 근거", "countermeasure": "대응방안"}}
        ]
