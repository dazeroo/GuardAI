...
WEB_CODE_MAP: Dict[str, dict] = {
    ...
    "AD": {"name": "관리자 페이지 노출", "countermeasure": "관리자 페이지 URL을 추측하기 어렵게 변경하고, 특정 IP 대역에서만 접근을 허용하며, 강력한 인증을 적용합니다. 단, 부득이하게 관리자 페이지를 외부에 노출해야 하는 경우 관리자 페이지 
로그인 시 2차 인증(otp, vpn, 인증서 등) 적용이 필요합니다."},
